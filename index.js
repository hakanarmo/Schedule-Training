...
//Grab all the command.js files from the commands folder.
fs.readdir("./commands/", (err, files) => {
  ...
  jsfile.forEach((f, i) =>{
    ...
    console.log(`OneNationGaming Bot File: ${f} Loaded!`);
...
bot.on("message", async message =>{
  ...
  if (message.channel.type == 'dm'){
    ...
    if(!active || !found ){
      ...
      let JrtheTeam = guild.roles.find('name', 'TheJrTeam');
      ...
      await channel.overwritePermissions(theTeam, {'READ_MESSAGES': true, 'SEND_MESSAGES': true,'MANAGE_MESSAGES': false,})
      await channel.overwritePermissions(JrtheTeam, {'READ_MESSAGES': true, 'SEND_MESSAGES': false,'MANAGE_MESSAGES': false,})
